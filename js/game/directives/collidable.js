(function(root, exports) {
	...
	Collidable = {
		...
		get: function(scope, name, sets) {
			if (!sets) {
				sets = this.sets(scope);
			}
			if (!sets[name]) {
		...
		directive: function(scope, element, attrs, $parse) {
			...
			for(var index = 0; index < len; index++) {
				this.get(scope, names[index].toLowerCase(), sets).add(object);
			}
			...
			for(var attr in attrs) {
				...
				var set = this.get(scope, attr.substring(7).toLowerCase(), sets);
			...
			element.bind('$destroy', function() {
				for(var index = 0; index < len; index++) {
					...
					sets[name].remove(attrs.collidable);
				}
